<template>
	<el-dialog :show-close="true" center :custom-class="'user-dialog'" :visible="dialogvisible" @close="$emit('update:dialogvisible', false)">
		<div class="avatar-box">
			<img :src="imagePlaceholder" alt="user avatar">
		</div>
		<div class="form-box">
			<el-input v-model="form.nombre" placeholder="Nombre" ></el-input>
			<el-input v-model="form.telefono" placeholder="Teléfono" ></el-input>
			<el-input v-model="form.direccion" placeholder="Dirección" ></el-input>
			<el-input v-model="form.ciudad" placeholder="Ciudad" ></el-input>
			<el-input v-model="form.pais" placeholder="País" ></el-input>
...
<script>
import { setContacts } from '@/api/user'
export default {
	name: 'ContactDialog',
	...
	methods: {
		submit(){
			this.form.id = 1
			setContacts(this.$store.getters.user.id, this.form).then(({data}) => {
				console.log(data)
